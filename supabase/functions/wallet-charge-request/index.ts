...
serve(async (req) => {
  ...
  try {
    ...
    const limited = await enforceRateLimit('wallet-charge-request', userSubject(user.id), ['notify_direct', 'notify_any'], corsHeaders);
    ...
    return json({ error: safeError('wallet-charge-request', err) }, 500);
  }
});
