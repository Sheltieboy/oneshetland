...
serve(async (req) => {
  ...
  try {
    ...
    if (!caller.isServiceRole) {
      const limited = await enforceRateLimit('notify-engagement', userSubject(caller.userId), ['notify_fanout', 'notify_any'], corsHeaders);
    ...
    return json({ error: safeError('notify-engagement', err) }, 500);
  }
});
