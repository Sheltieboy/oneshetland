...
serve(async (req) => {
  ...
  try {
    ...
    const limited = await enforceRateLimit('notify-collected', userSubject(user.id), ['notify_direct', 'notify_any'], corsHeaders);
    ...
    return new Response(
      JSON.stringify({ error: safeError('notify-collected', err) }),
