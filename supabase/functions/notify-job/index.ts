...
serve(async (req) => {
  ...
  try {
    ...
    if (!caller.isServiceRole) {
      const limited = await enforceRateLimit('notify-job', userSubject(caller.userId), ['notify_fanout', 'notify_any'], corsHeaders);
    ...
    return json({ error: safeError('notify-job', err) }, 500);
  }
});
