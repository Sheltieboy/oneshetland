...
serve(async (req) => {
  ...
  try {
    ...
    const limited = await enforceRateLimit('create-hub-membership-intent', userSubject(user.id), ['stripe_intent', 'stripe_any'], corsHeaders);
    ...
    if (use_saved_card) {
      ...
      if (outcome.kind === 'requires_action') {
        ...
        // grants membership when it succeeds.
      ...
      return json({ charged: true, status: 'succeeded', payment_intent_id: pi.id });
    }
