...
serve(async (req) => {
  ...
  try {
    ...
    const limited = await enforceRateLimit('notify-matching-workers', userSubject(user.id), ['notify_direct', 'notify_any'], corsHeaders);
    ...
    return new Response(
      JSON.stringify({ error: safeError('notify-matching-workers', err) }),
