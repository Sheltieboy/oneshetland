-- Business Transactions ledger (v1): one read-time UNION of every in-platform
-- money event for a business, normalised into a single exportable shape. No new
-- writes — always in sync with source tables. Owner-only (SECURITY DEFINER + an
-- explicit owner check, because the customer-owned source rows aren't readable
-- by the business under normal RLS).
--
-- Covered: wallet payments received, pass/class-pack sales, gift sales, booking
-- deposits, event-ticket sales (as organiser), and boosts paid (a cost). Platform
-- fee is itemised where the source stores it (wallet + tickets); shown as 0 where
-- it isn't yet persisted (pass/gift/booking) — those settle net via Stripe.
-- Subscriptions + bank payouts live in Stripe and are linked out from the UI.
...
BEGIN
  ...
  WITH ev AS (
    ...
    UNION ALL
    -- 6. Boosts paid (a cost to the business)
...
GRANT EXECUTE ON FUNCTION public.get_business_transactions(uuid, timestamptz, timestamptz, integer) TO authenticated;
