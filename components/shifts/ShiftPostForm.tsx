/**
 * ShiftPostForm.tsx — extracted from the legacy app/(tabs)/shifts.tsx.
 *
 * Contains the reusable "post a shift" building blocks:
 *   • PostShiftForm — the full shift-creation form
 *   • BoostSheet    — the post-success confirmation sheet
 * plus the module constants, generic form helpers and shared styles they
 * depend on. Lives outside app/ so Expo Router does NOT treat it as a route.
 */
...
// ── Post-shift confirmation sheet ─────────────────────────────────────────────
// Note: the in-app shift "boost" purchase has been removed for store
// compliance. This sheet now just confirms the shift went live. The
// `onBoosted` prop is retained for call-site compatibility but unused.
      ...
      <View style={styles.boostOverlay}>
        <View style={styles.boostSheet}>
          ...
          <Text style={styles.boostSheetBody}>
            Your shift is now live. Matching workers can find it in Shifts.
          </Text>

          <TouchableOpacity onPress={onDismiss} style={[styles.boostBtn, { marginTop: 8 }]} activeOpacity={0.85}>
            <FontAwesome5 name="check" size={13} color="#fff" solid />
            <Text style={styles.boostBtnText}>  Done</Text>
          </TouchableOpacity>
        </View>
      </View>
    </Modal>
